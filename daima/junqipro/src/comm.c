...
void SendLineup(Junqi* pJunqi, int iDir)
{
...
#endif
	{
		...
		header.reserve[0] = pJunqi->global.isAutoLineup;
...
void DealRecData(Junqi* pJunqi, u8 *data)
{
				...
				)
		{
			if( pJunqi->aBoard[p1.x][p1.y].pAdjList && pJunqi->aBoard[p2.x][p2.y].pAdjList )
			{
				...
				if( pSrc==NULL || pDst==NULL || pSrc->type==NONE )
				{
					...
					log_b("err1 %d %d %d %d",p1.x,p1.y,p2.x,p2.y);
					return;
				}
				if( pDst->type!=NONE && pSrc->pLineup->iDir%2==pDst->pLineup->iDir%2 )
				{
					SendHeader(pJunqi, pHead->iDir, COMM_ERROR);
					log_b("err2 %d %d %d %d",p2.x,p1.y,p2.x,p2.y);
					return;
				}
			}
			else
			{
				SendHeader(pJunqi, pHead->iDir, COMM_ERROR);
				log_b("err3 %d %d %d %d",p1.x,p1.y,p2.x,p2.y);
				return;
			}
			...
			{
				SendHeader(pJunqi, pHead->iDir, COMM_ERROR);
				log_b("err4 %d %d %d %d",p1.x,p1.y,p2.x,p2.y);
			}
		}
#ifdef AUTO_TEST
                AutoTest(pJunqi);
	...
	case COMM_LINEUP:
		//if( pHead->iDir%2==1 )
		{
		    pJunqi->global.isAutoLineup = 0;
			LoadLineup(pJunqi, pHead->iDir, (u8*)&pHead[1]);
			pJunqi->global.isAutoLineup = 1;
		}
		break;
	case COMM_REPLAY:
		break;
