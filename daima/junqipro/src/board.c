...
static void set_auto_Lineup(GtkWidget *item,gpointer data)
{
    Junqi *pJunqi = gJunqi;
    MenuData *pMenu;
    u8 now;

    pMenu = &pJunqi->menu.set_lineup;
    now = pMenu->nowSub;
    assert(now<pMenu->nSub);
    gtk_menu_item_set_label(GTK_MENU_ITEM(pMenu->apSubMenu[now]),
            pMenu->apLabel[now]);
    pMenu->nowSub = *((u8*)data);
    now = pMenu->nowSub;
    assert(now<pMenu->nSub);
    pMenu->aNowLabel[0] = '0';
    strcpy(pMenu->aNowLabel,pMenu->apLabel[now]);
    strcat(pMenu->aNowLabel,"√");
    gtk_menu_item_set_label(GTK_MENU_ITEM(pMenu->apSubMenu[now]),
            pMenu->aNowLabel);
    pJunqi->global.isAutoLineup = now;
    if(now){
        SendLineup(pJunqi, 0);
        SendLineup(pJunqi, 1);
    }
...
void CreatSetLineupMenu(GtkWidget *menu)
{
    GtkWidget *menuitem,*sub_menu;
    MenuData *pMenu;
    Junqi *pJunqi = gJunqi;
    //char *aLabe[2] = {"手动布阵","自动布阵"};
    static u8 aSelect[4] = {0,1,2,3};
    int i;

    menuitem = gtk_menu_item_new_with_label("布阵设置");
    gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
    sub_menu = gtk_menu_new();
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(menuitem),sub_menu);

    pMenu = &pJunqi->menu.set_lineup;
    pMenu->apLabel[0] = "手动布阵";
    pMenu->apLabel[1] = "自动布阵";
    for(i=0;i<2;i++){
        pMenu->apSubMenu[i] = SetMenuItem(sub_menu,pMenu->apLabel[i],
                set_auto_Lineup, (gpointer)&aSelect[i]);
    }
    pMenu->nSub = 2;
    pMenu->nowSub = 0;
}
...
void CreatDarkSelect(GtkWidget *menu)
{
    ...
    //gtk_menu_item_set_label(GTK_MENU_ITEM(tempItem), "全明√");
}
